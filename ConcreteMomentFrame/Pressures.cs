...
namespace CivilApp.ConcreteMomentFrame
{
    class Pressures
    {
        ...
        public Loads CalculateLoads(Item area)
        {
            Loads loads = new Loads();

            foreach (Item pressure in QList)
            {

                Item ans = ItemMath.Multiply(pressure, area);
                Item load = new Item(pressure.name, ans.number, "kN", pressure.note + " " + ans.note);
                loads.AddQ(load);
            }

            foreach (Item pressure in GList)
            {
                Item ans = ItemMath.Multiply(pressure, area);
                Item load = new Item(pressure.name, ans.number, "kN", pressure.note + " " + ans.note);
                loads.AddG(load);
        ...
        {
            Item item = new Item("Total G", FindTotal(GList) * factor, "kN/m2", "factor:" + factor);
            return item;
        }

        public Item TotalQ(double factor)
        {
            Item item = new Item("Total Q", FindTotal(QList) * factor, "kN/m2", "factor:" + factor);
            return item;
        }
